...
use crate::constants::{
    ...
    HF_SHARD_STORE_SOURCE_SIZE_KEY, HF_SHARED_RUNTIME_WORKER_THREADS, HF_TEMP_DEFAULT_EXTENSION,
    HF_TEMP_DOWNLOAD_PREFIX, HF_WHOAMI_DEFAULT_ENDPOINT,
};
...
pub(crate) fn hf_source_id_slug(dataset: &str, config: &str, split: &str) -> String {
...
impl HuggingFaceRowSource {
    ...
    pub(crate) fn candidate_store_path(config: &HuggingFaceRowsConfig, candidate: &str) -> PathBuf {
        shard_store_path_for(&Self::candidate_target_path(config, candidate))
    }
    ...
    pub(crate) fn read_store_row_count(&self, store: &DataStore) -> Result<usize, SamplerError> {
    ...
    pub(crate) fn download_next_remote_shard(&self) -> Result<bool, SamplerError> {
        ...
        if is_store_shard_path(&shard.path) {
            let canonical_store =
                shard_store_path_for(&Self::candidate_target_path(&self.config, &remote_path));
