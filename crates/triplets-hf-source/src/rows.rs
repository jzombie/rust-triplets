...
pub(crate) fn read_row_batch(
    ...
    if !pending.is_empty() {
        let resolutions = {
            ...
            for idx in &pending {
                let (shard, local_idx) = HuggingFaceRowSource::locate_shard(&state.shards, *idx)
                    .ok_or_else(|| SamplerError::SourceUnavailable {
                        source_id: source.config.source_id.clone(),
        ...
        for (idx, shard, local_idx) in resolutions {
            let (group_pos, local_in_group) = source.locate_parquet_group(&shard, local_idx)?;
            parquet_groups
        ...
        for ((shard_path, group_pos), mut requested) in parquet_groups {
            ...
            if is_store_shard_path(&shard.path) {
                let store = source.get_or_open_shard_store(&shard.path)?;
...
pub(crate) fn transcode_transient_shard_to_store(
    ...
    let store = source.get_or_open_shard_store(&store_path)?;
