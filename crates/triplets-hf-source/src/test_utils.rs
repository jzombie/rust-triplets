...
/// Calling them concurrently with any other env-var read or write (including
/// implicit reads via `std::process::Command` or `reqwest` proxy resolution)
/// can cause undefined behaviour.  In practice env-var mutations are safe in
/// single-threaded test code, but the `unsafe` keyword is required because
/// the compiler cannot prove that no concurrent access exists.
///
/// **Usage rules:**
/// - Only use in test code (`#[cfg(test)]` or integration tests).
/// - Tests that mutate the **same** env var concurrently must be annotated
///   with `#[serial_test::serial]` to prevent interleaving.
/// - Tests that mutate **different** env vars can run in parallel safely.
/// - Keep the guard alive for the entire async scope so that background
///   threads (e.g. HF expansion workers) see the intended value.
///
/// # Availability
///
/// This type is only compiled when the `test-utils` feature is enabled
/// (or when `cfg(test)` applies).  It is unconditionally safe to depend on
/// from integration tests because the `Cargo.toml` dev-dependencies enable
...
impl EnvGuard {
    ...
    /// # Safety
    ///
    /// See the [struct-level safety docs](Self#safety).
    pub fn set(key: &str, value: &str) -> Self {
        let previous = std::env::var(key).ok();
        // SAFETY: EnvGuard is only constructed in test code where env-var
        // mutations are safe under the rules documented above.  Concurrent
        // access to the same env var is prevented via `#[serial_test::serial]`.
...
impl Drop for EnvGuard {
    fn drop(&mut self) {
        // SAFETY: See the struct-level safety docs.  Drop runs on the same
        // thread that constructed the guard, serialised by the caller's
        // test harness or #[serial_test::serial].
