...
>
> **On hard negative mining:** By default, hardness comes from metadata/recipe constraints and in-source negatives, which keeps the hot path lightweight and predictable for debugging and throughput. The optional `bm25-mining` feature adds BM25-based lexical ranking within strategy-defined candidate pools — BM25 scores are used only for ranking candidates already selected by your recipe strategy, not as a global filter. This is built into the crate's scoring pipeline. For embedding-based retrieval or offline pre-ranking, those can be integrated separately via pre-ranked sources or per-batch source reweighting.
