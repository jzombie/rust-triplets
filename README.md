...
> Note on "hard triplet" mining: by default, hardness comes from metadata/recipe constraints and in-source negatives, which keeps the hot path lightweight and predictable for debugging and throughput. When you need stronger lexical negatives, enable optional BM25 mining (`bm25-mining` feature): candidate pools are still defined by your selected negative strategy, and BM25 ranks candidates within those pools.
>
...
- **Optional BM25 hard-negative mining**: rule-based sampling remains the default fast path for iteration/debug and high-throughput runs. When explicit lexical hard negatives are needed, enable the optional `bm25-mining` feature to rank same-split candidates inside each strategy-defined pool. Because negatives are mined per-source by default, each source is still treated as a domain boundary unless your source/recipe design intentionally broadens it.
